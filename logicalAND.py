#compute the P masks by thresholding with usup(k)
...
def mean_sup(M,N,f,count,output_path):
	...
	#thresholding using the mean_sup value

	binsupf = np.zeros((M,N))

	for i in range(0,M-1):
		for j in range(0,N-1):
			binsupf[i][j] = int(f[i][j] > mean_sup)
	...
	#binsup_img.show()
	cv2.imwrite(os.path.join(output_path,'binsupimg{}.png'.format(count)),binsupf)
...
for k_image in image_folder:
	...
	#print(count)
print("Bye")
#access the P masks and compute the logical AND

#mask_folder = os.listdir(output_path)

#mask_array = np.zeros()
#for i in range(0,P-1):
#	mask_array = int(mask_folder[i] & mask_folder[i+1])
